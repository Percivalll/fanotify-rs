...
impl FanotifyEventMetadata {
    pub fn from_reader(mut reader: impl std::io::Read) -> std::io::Result<Self> {
        let mut event_meta_buffer = [0u8; size_of::<Self>()];
        reader.read_exact(&mut event_meta_buffer)?;
        // SAFETY: we know that all bit paterns of `FanotifyEventMetadata` are the valid, and the read succeeded, so a memory cast is safe.
        Ok(unsafe { core::mem::transmute(event_meta_buffer)})
    }
...
pub fn fanotify_read(fanotify_fd: i32) -> Vec<(FanotifyEventMetadata, Vec<FanAdditionalRecords>)> {
    ...
    if read_len == (0-libc::EAGAIN) as isize || read_len <= 0 { return vec![];  }

    let mut cursor = read_buffer.as_ref().as_slice();
    while let Ok(event_metadata) = FanotifyEventMetadata::from_reader(cursor) {
