...
impl FanotifyEventMetadata {
    pub fn from_slice(slice: &[u8]) -> std::io::Result<Self> {
        if slice.len() < size_of::<Self>() {
            return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "The buffer is too small to contain a FanotifyEventMetadata"))
        }
        // SAFETY: we know that all bit paterns of `FanotifyEventMetadata` are the valid,
        // and there is enough space in the buffer to contain the `FanotifyEventMetadata`
        // so a memory cast and read is safe.
        Ok(unsafe {slice.as_ptr().cast::<Self>().read()})
    }
}

/// The different types of fanotify information records that can be returned with an event.
#[derive(Debug, Clone, Copy)]
#[repr(u8)]
pub enum FanoityEnventInfoType {
    FID = 1,
    DFIDName = 2,
    DFID = 3,
    PIDFD = 4,
...
/// The header indentifying what type of information event was generated by the fanotify subsystem, and the size of the struct (for the `fid` DST).
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub struct FanotifyEventInfoHeader {
    /// The type of information event that was generated by the fanotify subsystem.
    info_type: FanoityEnventInfoType,
    /// Padding to align the u16 length field.
    _pad: __u8,
    /// The size of the additional information record, including this header.
    len: __u16
}

/// Container class to encapsulate the different types of fanotify information records that can be returned with an event
#[derive(Debug, Clone)]
#[repr(C)]
pub enum FanAdditionalRecords {
    FanotifyEventInfoFID(FanotifyEventInfoFID),
    FanotifyEventInfoDFIDName{fid: FanotifyEventInfoFID, name: CString},
    FanotifyEventInfoPIDFD(FanotifyEventInfoPIDFD),
    FanotifyEventInfoError(FanotifyEventInfoError)
}

impl FanAdditionalRecords {
    pub fn size(&self) -> usize {
        match self {
            Self::FanotifyEventInfoFID(ref e) => e.header.len as usize,
            Self::FanotifyEventInfoDFIDName{ref fid, ref name } => fid.header.len as usize + name.as_bytes_with_nul().len(),
            Self::FanotifyEventInfoPIDFD(ref e) => e.header.len as usize,
            Self::FanotifyEventInfoError(ref e) => e.header.len as usize
        }
    }

    pub fn from_slice(slice: &[u8]) -> Result<(Self, usize), ()> {
        if slice.len() < size_of::<FanotifyEventInfoHeader>() {
            return Err(());
        }
        // SAFETY: We definitely have enough bytes so we can safely read.
        match unsafe{ *(slice.as_ptr().cast())} {
            FanoityEnventInfoType::FID | FanoityEnventInfoType::DFID => {
                let (fid, size) = FanotifyEventInfoFID::from_slice(slice).map_err(|_| ())?;
                Ok((Self::FanotifyEventInfoFID(fid), size))
            },
            FanoityEnventInfoType::DFIDName => {
                let (fid, size) = FanotifyEventInfoFID::from_slice(slice).map_err(|_| ())?;
                if slice.len() < size {
                    return Err(());
                }
                ...
                let name: CString = unsafe {CStr::from_ptr(slice.as_ptr() as _)}.to_owned();
                let consumed_bytes = size + name.as_bytes_with_nul().len();
                Ok((Self::FanotifyEventInfoDFIDName{fid, name},
                    consumed_bytes
                ))
            },
            FanoityEnventInfoType::PIDFD => {
                Ok((Self::FanotifyEventInfoPIDFD(unsafe {slice.as_ptr().cast::<FanotifyEventInfoPIDFD>().read_unaligned()}), size_of::<FanotifyEventInfoPIDFD>()))
            },
            FanoityEnventInfoType::Error => {
                Ok((Self::FanotifyEventInfoError(unsafe {slice.as_ptr().cast::<FanotifyEventInfoError>().read_unaligned()}), size_of::<FanotifyEventInfoError>()))
            }
        }
    }
}

/// Reporting event for File System Errors when `FAN_REPORT_FSERROR` is reported in the event mask
#[derive(Debug, Clone, Copy)]
#[repr(C)]
pub struct FanotifyEventInfoError {
    pub header: FanotifyEventInfoHeader,
    pub error_code: __s32,
    pub error_count: __u32
}

/// Additional information record containing a file descriptor for the target process.
#[derive(Debug)]
#[repr(C)]
...
impl Clone for FanotifyEventInfoPIDFD {
    fn clone(&self) -> Self {
        unsafe {
            Self {
                header: self.header,
                pidfd: libc::dup(self.pidfd)
            }
        }
    }
}

impl Drop for FanotifyEventInfoPIDFD {
    fn drop(&mut self) {
        unsafe {
...
#[derive(Debug, Clone)]
#[repr(C)]
struct OpaqueFileHandle {
    handle_bytes: u32,
    handle_type: i32,
    file_handle: Vec<u8>
}

impl OpaqueFileHandle {
    fn encoded_size(&self) -> usize {
        size_of::<u32>() + size_of::<i32>() + self.file_handle.len()
    }

    fn from_slice(mut slice: &[u8]) -> Result<(Self, usize), Error> {
        if slice.len() < 8 {
            return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "The buffer is too small to contain a file handle"))
        }

        let handle_bytes = u32::from_ne_bytes(slice[..4].try_into().expect("We already checked the length, so this should never fail."));
        let min_size = handle_bytes as usize + size_of::<u32>() + size_of::<i32>();
        if slice.len() < min_size {
            return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, format!("The buffer is too small to contain a file handle with the declared length {}", handle_bytes)))
        }
        slice = &slice[4..];
        
        let handle_type = i32::from_ne_bytes(slice[..4].try_into().expect("We already checked the length, so this should never fail."));
        slice = &slice[4..];
        let file_handle = Vec::from(&slice[..handle_bytes as usize]);

        Ok((Self {handle_bytes, handle_type, file_handle}, handle_bytes as usize + size_of::<u32>() + size_of::<i32>()))
    }
}



pub type KernelFSID = [libc::c_int;2];
...
pub struct FanotifyEventInfoFID {
    pub header: FanotifyEventInfoHeader,
    pub fsid: KernelFSID,
    handle: OpaqueFileHandle
}

impl FanotifyEventInfoFID {
    fn from_slice(slice: &[u8]) -> Result<(Self, usize), Error> {
        if slice.len() < size_of::<FanotifyEventInfoHeader>() + size_of::<KernelFSID>() {
            return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "The buffer is too small to contain a FID record"))
        }

        let ptr = slice.as_ptr().cast::<Self>();
        // SAFETY: We have already checked that there are enough bytes in the buffer
        let (header, fsid) = unsafe {
            ((*ptr).header, (*ptr).fsid)
        };

        if slice.len() < header.len as usize {
            return Err(std::io::Error::new(std::io::ErrorKind::UnexpectedEof, "The buffer is too small to contain the full FID record"))
        }

        let (handle, handle_size) = OpaqueFileHandle::from_slice(&slice[12..])?;

        Ok((Self{ header, fsid, handle}, size_of::<FanotifyEventInfoHeader>() + size_of::<KernelFSID>() + handle_size))
    }
}
...
const FAN_EVENT_METADATA_LEN: usize = size_of::<FanotifyEventMetadata>();
...
/// Flags to determine fanotify event format
/// report unique directory id and path name.
...
pub fn fanotify_read(fanotify_fd: i32) -> Vec<(FanotifyEventMetadata, Vec<FanAdditionalRecords>)> {
    let mut events = Vec::new();
    let mut read_buffer = Box::new([0u8;FAN_EVENT_METADATA_LEN * 200]);
    let read_len = unsafe { libc::read(fanotify_fd, read_buffer.as_mut_ptr() as _, FAN_EVENT_METADATA_LEN * 200)};
    if read_len == libc::EAGAIN as isize || read_len <= 0 { return vec![];  }
    ...
    while let Ok(event_metadata) = FanotifyEventMetadata::from_slice(cursor) {
        ...
        if event_metadata.event_len as usize > FAN_EVENT_METADATA_LEN {
            ...
            while inner_cursor.len() >= size_of::<FanotifyEventInfoHeader>() {
                ...
                if let Ok((additional_info, slice_consumed)) = FanAdditionalRecords::from_slice(inner_cursor)  {
                    inner_cursor = &inner_cursor[slice_consumed..];
                    extra_records.push(additional_info);
                } else {
                    break; // this only happens when there wansn't enough space to read the events that should have been there, so we just stop here.
                }
            }
            
        }
        events.push((event_metadata, extra_records));
        cursor = &cursor[event_metadata.event_len as usize ..];
    }
    events
}
