...
pub struct FanotifyEventMetadata {
    ...
    /// buffer.
    ///
    /// Without `FAN_REPORT_FID`, the value of event_len is always `FAN_EVENT_METADATA_LEN`.
    ///
    /// With `FAN_REPORT_FID`, event_len also includes the variable length file identifier.
    pub event_len: __u32,

    /// This field holds a version number for the structure.
    ///
    /// It must be compared to `FANOTIFY_METADATA_VERSION` to verify that the structures returned at
    /// run time match the structures defined at compile time.
    ///
    /// In case of a mismatch, the application should abandon trying to use the fanotify file
    /// descriptor.
    pub vers: __u8,
    ...
    ///
    /// The field was introduced to facilitate the implementation of optional headers per event
    /// type.
    ///
    ...
    /// This is an open file descriptor for the object being accessed, or `FAN_NOFD` if a queue
    /// overflow occurred.
    ///
    /// If the fanotify file descriptor has been initialized using `FAN_REPORT_FID`, applications
    /// should expect this value to be set to `FAN_NOFD` for each event that is received.
    ///
    /// The file descriptor can be used to access the contents of the monitored file or directory.
    ///
    /// The reading application is responsible for closing this file descriptor. When calling
    /// `fanotify_init(2)`, the caller may specify (via the `event_f_flags` argument) various file
    /// status flags that are to be set on the open file description that corresponds to this file
    /// descriptor.
    ///
    /// In addition, the (kernel-internal) `FMODE_NONOTIFY` file status flag is set on the open file
    /// description. This flag suppresses fanotify event generation. Hence, when the receiver of the
    /// fanotify event accesses the notified file or directory using this file descriptor, no
    /// additional events will be created.
    pub fd: __s32,

    /// If the flag `FAN_REPORT_TID` was set in `fanotify_init(2)`, this is the TID of the thread
    /// that caused the event. Otherwise, this the PID of the process that caused the event.
    pub pid: __s32,
}
...
/// This const is used to be compared to the `vers` field of `FanotifyEventMetadata` to verify that
/// the structures returned at run time match the structures defined at compile time.
///
/// In case of a mismatch, the application should abandon trying to use the fanotify file
/// descriptor.
...
///
/// Reading from the file descriptor will not block. Instead, if no data is available, `read(2)`
...
/// permission decisions if a file may be accessed.
///
/// It is intended for event listeners that need to access files when they already contain their
/// final content.
///
...
/// permission decisions if a file may be accessed.
///
/// It is intended for event listeners that need to access files before they contain their final
/// data.
///
...
///
/// An fanotify file descriptor created with `FAN_CLASS_PRE_CONTENT` or `FAN_CLASS_CONTENT` is
/// required.
...
/// `FAN_MOVED_TO`, specifying the flag `FAN_ONDIR` is required in order to create events when
...
///
/// More specifically, the directory entry modification events `FAN_CREATE`, `FAN_DELETE`,
/// `FAN_MOVED_FROM`, and `FAN_MOVED_TO` are not generated for any entry modifications performed
/// inside subdirectories of marked directories.
///
/// Note that the events `FAN_DELETE_SELF` and `FAN_MOVE_SELF` are not generated for children of
/// marked directories.
///
...
///
/// No flag other than and at most one of the flags `FAN_MARK_MOUNT` or `FAN_MARK_FILESYSTEM` can be
/// used in conjunction with `FAN_MARK_FLUSH`.
///
/// `mask` is ignored.
...
///
/// The events which require the `fanotify_fd` file descriptor to have been initialized with the
/// flag `FAN_REPORT_FID`, such as `FAN_CREATE`, `FAN_ATTRIB`, `FAN_MOVE`, and `FAN_DELETE_SELF`,
...
///
/// If this flag is not set, the ignore mask is cleared when a modify event occurs for the ignored
/// file or directory.
...
/// Initializes a new fanotify group and returns a file descriptor for the event queue associated
/// with the group.
///
/// The file descriptor is used in the calls to `fanotify_mark(2)` to specify the files,
/// directories, mounts or filesystems for which fanotify events shall be created. These events are
/// received by reading from the file descriptor.
///
/// Some events are only informative, indicating that a file has been accessed. Other events can be
/// used to determine whether another application is permitted to access a file or directory.
/// Permission to access filesystem objects is granted by writing to the file descriptor. Multiple
/// programs may be using the fanotify interface at the same time to monitor the same files.
///
/// In the current implementation, the number of fanotify groups per user is limited to 128. This
/// limit cannot be overridden. Calling `fanotify_init()` requires the `CAP_SYS_ADMIN` capability.
/// This constraint might be relaxed in future versions of the API. Therefore, certain additional
...
/// descriptions that are created for fanotify events.
///
/// For details of these flags, see the description of the flags values in `open(2)`.
///
/// `event_f_flags` includes a multi-bit field for the access mode.
///
...
/// Specifying any other flag in `event_f_flags` yields the error `EINVAL`.
///
/// # Examples
///
/// ```
...
///
/// `flags` is a bit mask describing the modification to perform. It must include exactly one of the
/// following values:
/// * `FAN_MARK_ADD`
/// * `FAN_MARK_REMOVE`
/// * `FAN_MARK_FLUSH`
...
///
/// `mask` defines which events shall be listened for (or which shall be ignored). It is a bit mask
...
///
/// The filesystem object to be marked is determined by the file descriptor `dirfd` and the pathname
...
/// ignored.
/// * If pathname is relative, and `dirfd` does not have the value `AT_FDCWD`, then the filesystem
/// object to be marked is determined by interpreting pathname relative the directory referred to by
/// `dirfd.`
/// * If pathname is relative, and `dirfd` has the value `AT_FDCWD`, then the filesystem object to
/// be marked is determined by interpreting pathname relative the current working directory.
///
/// # Examples
///
/// ```
