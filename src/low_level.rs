...
impl TryFrom<&[u8]> for FanotifyEventMetadata {
    type Error = std::io::Error;
    fn try_from(slice: &[u8]) -> Result<Self, Self::Error> {
        if slice.len() < size_of::<Self>() {
            return Err(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
                "The buffer is too small to contain a FanotifyEventMetadata",
            ));
        }
        ...
        Ok(unsafe { slice.as_ptr().cast::<Self>().read_unaligned() })
    }
}
...
impl FanAdditionalRecords {
    pub fn size(&self) -> usize {
        match self {
            Self::FanotifyEventInfoFID(e) => e.header.len as usize,
            Self::FanotifyEventInfoDFIDName { fid, name } => {
                fid.header.len as usize + name.as_bytes_with_nul().len()
            }
            Self::FanotifyEventInfoPIDFD(e) => e.header.len as usize,
            Self::FanotifyEventInfoError(e) => e.header.len as usize,
        }
    }

    pub fn from_slice(slice: &[u8]) -> std::io::Result<(Self, usize)> {
        if slice.len() < size_of::<FanotifyEventInfoHeader>() {
            return Err(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
                "The buffer is too small to contain an event info header record",
            ));
        };
        ...
        match unsafe { *(slice.as_ptr().cast()) } {
            FanoityEnventInfoType::FID | FanoityEnventInfoType::DFID => {
                let (fid, size) = FanotifyEventInfoFID::from_slice(slice)?;
            ...
            FanoityEnventInfoType::DFIDName => {
                ...
                if slice.len() < size {
                    return Err(std::io::Error::new(
                        std::io::ErrorKind::UnexpectedEof,
                        "The buffer is too small to contain an the expected DFIDName record",
                    ));
                }
                // SAFETY: We definitely have enough bytes so we can safely read.
            ...
            FanoityEnventInfoType::PIDFD => Ok((
                Self::FanotifyEventInfoPIDFD(unsafe {
                    slice
                        .as_ptr()
                        .cast::<FanotifyEventInfoPIDFD>()
                        .read_unaligned()
                }),
                size_of::<FanotifyEventInfoPIDFD>(),
            )),
            FanoityEnventInfoType::Error => Ok((
                Self::FanotifyEventInfoError(unsafe {
                    slice
                        .as_ptr()
                        .cast::<FanotifyEventInfoError>()
                        .read_unaligned()
                }),
                size_of::<FanotifyEventInfoError>(),
            )),
        }
    }
}
...
impl OpaqueFileHandle {
    fn from_slice(mut slice: &[u8]) -> Result<(Self, usize), Error> {
        if slice.len() < 8 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
                "The buffer is too small to contain a file handle",
            ));
        }
        ...
        let handle_bytes = u32::from_ne_bytes(
            slice[..4]
                .try_into()
                .expect("We already checked the length, so this should never fail."),
        );
        ...
        if slice.len() < min_size {
            return Err(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
                format!(
                    "The buffer is too small to contain a file handle with the declared length {}",
                    handle_bytes
                ),
            ));
        }
        ...
        Ok((
            ...
            handle_bytes as usize + size_of::<u32>() + size_of::<i32>(),
        ))
    }
}
...
impl FanotifyEventInfoFID {
    fn from_slice(slice: &[u8]) -> Result<(Self, usize), Error> {
        if slice.len() < size_of::<FanotifyEventInfoHeader>() + size_of::<KernelFSID>() {
            return Err(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
                "The buffer is too small to contain a FID record",
            ));
        }

        let ptr = slice.as_ptr().cast::<Self>();
        // SAFETY: We have already checked that there are enough bytes in the buffer
        let (header, fsid) = unsafe { ((*ptr).header, (*ptr).fsid) };

        if slice.len() < header.len as usize {
            return Err(std::io::Error::new(
                std::io::ErrorKind::UnexpectedEof,
                "The buffer is too small to contain the full FID record",
            ));
        }
        ...
        Ok((
            ...
            size_of::<FanotifyEventInfoHeader>() + size_of::<KernelFSID>() + handle_size,
        ))
    }
}
...
pub fn fanotify_read(
    fanotify_fd: i32,
) -> std::io::Result<Vec<(FanotifyEventMetadata, Vec<FanAdditionalRecords>)>> {
    ...
    while let Ok(event_metadata) = FanotifyEventMetadata::try_from(cursor) {
        ...
        if event_metadata.event_len as usize > FAN_EVENT_METADATA_LEN {
            ...
            let mut inner_cursor =
                &cursor[FAN_EVENT_METADATA_LEN..event_metadata.event_len as usize];
            while inner_cursor.len() >= size_of::<FanotifyEventInfoHeader>() {
                ...
                if let Ok((additional_info, slice_consumed)) =
                    FanAdditionalRecords::from_slice(inner_cursor)
                {
