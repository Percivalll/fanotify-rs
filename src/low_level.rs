...
impl FanAdditionalRecords {
    ...
    pub fn from_slice(slice: &[u8]) -> Result<Self, ()> {
        ...
        let header: FanotifyEventInfoHeader = unsafe {slice.as_ptr().cast::<FanotifyEventInfoHeader>().read_unaligned()};
        ...
        match header.info_type {
            FanoityEnventInfoType::FID | FanoityEnventInfoType::DFID | FanoityEnventInfoType::DFIDName => {
                // SAFETY: We definitely have enough bytes so we can safely read.
                let fsid: __s32 = unsafe{ slice[offset_of!(FanotifyEventInfoFID, fsid)..].as_ptr().cast::<__s32>().read_unaligned()};
                // SAFETY: We only read bytes from a validated range so we can safely read.
                let mut handle: Vec<u8> = Vec::with_capacity(header.len as usize - offset_of!(FanotifyEventInfoFID, handle));
                handle.extend_from_slice(&slice[offset_of!(FanotifyEventInfoFID, handle)..header.len as usize]);
                Ok(Self::FanotifyEventInfoFID(FanotifyEventInfoFID {
                    header,
                    fsid,
            ...
            FanoityEnventInfoType::PIDFD => {
                Ok(Self::FanotifyEventInfoPIDFD(unsafe {slice.as_ptr().cast::<FanotifyEventInfoPIDFD>().read_unaligned()}))
            },
            FanoityEnventInfoType::Error => {
                Ok(Self::FanotifyEventInfoError(unsafe {slice.as_ptr().cast::<FanotifyEventInfoError>().read_unaligned()}))
            }
        }
    }
}
    ...
    while let Ok(event_metadata) = FanotifyEventMetadata::from_slice(cursor) {
        ...
        if event_metadata.event_len as usize > FAN_EVENT_METADATA_LEN {
            ...
            while inner_cursor.len() >= size_of::<FanotifyEventInfoHeader>() {
                ...
                if let Ok(additional_info) = FanAdditionalRecords::from_slice(inner_cursor)  {
                    inner_cursor = &inner_cursor[additional_info.size()..];
